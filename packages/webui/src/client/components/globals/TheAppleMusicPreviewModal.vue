...
const allSelected = computed(() =>
  playlist.value ? selectedIndices.value.size === playlist.value.tracks.length : false
);
...
function download() {
  ...
  toast(`Adding ${nSelected} track${nSelected !== 1 ? "s" : ""} to queue…`, "loading", false, "apple_dl");
  sendAddToQueue(urlToQueue);
  closeModal();
}

onMounted(() => { emitter.on("AppleMusicPreview:open", openModal); });
onUnmounted(() => { emitter.off("AppleMusicPreview:open", openModal); });
</script>
    ...
    <div class="apple-music-modal">
      ...
      <template v-else-if="playlist">
        ...
        <div class="modal-header">
          <img v-if="playlist.artworkUrl" :src="playlist.artworkUrl" alt="artwork" class="modal-artwork" />
          <div>
            <h2 class="modal-title">{{ playlist.title }}</h2>
            <p class="modal-curator" v-if="playlist.curator">{{ playlist.curator }}</p>
          </div>
        ...
        <div class="modal-controls">
          ...
          <span class="modal-count">{{ selectedCount }} / {{ playlist.tracks.length }} selected</span>
        </div>
...
.modal-controls button:hover { background: rgba(255,255,255,0.15); }
.modal-count { margin-left: auto; opacity: 0.5; }
.modal-tracklist {
  list-style: none;
  margin: 0;
  padding: 0.25rem 0;
...
.modal-track:hover { background: rgba(255,255,255,0.05); }
.modal-track input[type="checkbox"] { cursor: pointer; flex-shrink: 0; }
.track-num { opacity: 0.4; font-size: 0.8rem; min-width: 1.8rem; text-align: right; }
...
.track-artist { font-size: 0.78rem; opacity: 0.55; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.modal-footer {
  display: flex;
  gap: 0.75rem;
...
.btn-download:not(:disabled):hover { filter: brightness(1.1); }
